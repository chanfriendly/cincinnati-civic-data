...
// ─── Neighborhood Lead Service Line Stats ─────────────────────────────────────

let _leadCachePromise: Promise<Map<string, NeighborhoodLeadStats>> | null = null;

/**
 * Load pre-computed GCWW lead service line inventory for every Cincinnati neighborhood.
 * Returns a Map keyed by stripNeighborhoodName() output (lowercase alphanumeric).
 *
 * Data source: /data/lead_service_lines.json
 * Generated by: scripts/build_lead.py (run locally — GCWW API may be blocked from
 * the Cowork sandbox due to network restrictions).
 *
 * If the file is empty ({}) the returned Map will be empty. The Lead Safety tab
 * handles this gracefully with a "run build script" notice.
 */
export async function fetchNeighborhoodLeadStats(): Promise<Map<string, NeighborhoodLeadStats>> {
  if (_leadCachePromise) return _leadCachePromise;

  _leadCachePromise = (async () => {
    const resp = await fetch('/data/lead_service_lines.json');
    if (!resp.ok) throw new Error(`Lead data file not found: HTTP ${resp.status}`);
    const raw: Record<string, NeighborhoodLeadStats> = await resp.json();
    const result = new Map<string, NeighborhoodLeadStats>();
    for (const [key, value] of Object.entries(raw)) {
      result.set(key, value);
    }
    return result;
