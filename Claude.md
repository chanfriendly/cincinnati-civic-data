...
22. ✅ **Limitations tab — community contributions** — Google Form wired into the About & Methods tab (Section 8 "Report an issue or contribute"). Three contribution types: Data Error, Primary Source Citation, Dataset Suggestion. Form URL: **https://forms.gle/sMHyvc4Hu8FMwARE8**. Responses go to a Google Sheet; email notifications enabled. URL is stored in `const GOOGLE_FORM_URL` at the top of `src/tabs/Limitations/index.tsx` — update that one constant if the form ever changes.
...
24. ✅ **Life expectancy by neighborhood** — `public/data/neighborhood_life_expectancy.json` (41 neighborhoods, built by `scripts/build_life_expectancy.py` from CDC USALEEP FTP CSV). `LifeExpectancySection.tsx` wired into Neighborhood Profiles under "Public Health" divider before HealthOutcomesSection. Shows life expectancy at birth, city-range gradient bar, ±years vs city avg badge, and equity callout (23-year gap across Cincinnati neighborhoods). Single-tract estimates flagged as lower confidence.
...
26. ✅ **Community/neighborhood councils directory** — `public/data/community_councils.json` (52 entries, manually curated from Google Sheets directory). `CommunityCouncilSection.tsx` wired into Neighborhood Profiles under "Community & Civic" divider. Includes inactive flag, multi-neighborhood CCBs (CUF), GitHub correction link.
27. ✅ **Voting precinct lookup** — Live CAGIS FeatureServer layer 44 point-in-polygon query. Shows precinct name, polling location, address. Links to Hamilton County BOE voter registration lookup. Added to Address Lookup under "Politics & Government" section.
28. ✅ **Recreation centers (CRC)** — `public/data/recreation_centers.json` (24 CRC centers, geocoded via OSM Nominatim). `RecreationCentersSection.tsx` wired into Neighborhood Profiles under "Community & Civic" divider. Falls back to all-centers list if none assigned to selected neighborhood.
29. ✅ **Expanded demographics** — `public/data/neighborhood_demographics.json` (41 neighborhoods, built by `scripts/build_demographics.py`). `ExpandedDemographicsSection.tsx` wired into Neighborhood Profiles. Shows age structure, language (C16001 — B16001 suppressed at tract level), foreign-born, education, household type. City-avg comparison badges throughout.
30. ✅ **Broadband access rates** — Included in `neighborhood_demographics.json` (ACS B28002: broadband % + no-internet %). Displayed in `ExpandedDemographicsSection.tsx` under "Internet Access" with city-avg comparison badge.
